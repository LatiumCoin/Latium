...
// minimum age for coin age: 1h
unsigned int nStakeMinAge = 60 * 60;
// stake age of full weight: 7d NOTE: This is only factored into the difficulty of creating a block, not interest calculations.
unsigned int nStakeMaxAge = 60 * 60 * 24 * 7;
// 150 sec block spacing
unsigned int nStakeTargetSpacing = 150;

int64 nChainStartTime = 1398349858;
...
const string strMessageMagic = "Latium Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+8) - GetDepthInMainChain());
}
...
int64 GetProofOfWorkReward(int nHeight) {
	...
	// Premine in two blocks, due to initial misunderstanding of maximum number of coins -> PoS coins have less decimal places.
	return (nHeight == 1 || nHeight == 2) ? PREMINE / 2 : CENT;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight) {
    ...
    if (nHeight < BLOCKS_PER_DAY * 90)
        interest = 10; // 50%
    else if (nHeight < BLOCKS_PER_DAY * 365)
        interest = 4; // 20%
    else
        interest = 1; // 5%
    
    // We want some accuracy and yet to avoid integer overflows, so multiply by smallest amount possible, factoring down COIN/365/20 into CENT/73. Multiply interest last.
    int64 nSubsidy = nCoinAge * CENT / 73 * interest;
...
static const int64 nTargetTimespan = 30*150; // 75 minutes
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Latium
    bool fStrictPayToScriptHash = true; // Always active in Latium
	...
	if (vtx[0].GetValueOut() > GetProofOfWorkReward(pindex->nHeight))
		return false;
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Check proof of work matches claimed amount
    if (fCheckPOW && IsProofOfWork() && hashPrevBlock != 0 && !CheckProofOfWork(GetHash(), nBits))
    ...
    if (hashPrevBlock != 0 && !CheckBlockSignature())
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Latium", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "14/04/2014 French finance minister Michel Sapin says taxes were raised too high";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("d36470050116775c8da910f145fbbbc680fbb4bf78c47b51d07b82cba4df89a5"));
...
unsigned char pchMessageStart[4] = { 0xae, 0xdb, 0xfc, 0xb2 };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    {
        pfrom->Misbehaving(20);
        return error("No checkpointing allowed!");
    }
